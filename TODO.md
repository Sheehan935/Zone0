...
- [ ] **Photo Check — R2 cleanup**: Delete the 9 pre-existing test objects in `zone0-photo-check-uploads`, plus two more added 2026-08-25 during redesign deploy verification (lead ids `98e8b645-4d17-43d0-8d2e-75eb2fafdfd2` "QA TEST - DO NOT CONTACT" and `f5d474c4-84e3-4e57-b306-aef78abb96c1` "QA TEST 2 - DO NOT CONTACT") — 11 total (see `docs/PROJECT-STATE.md`).
- [x] **Review Portal — first production workflow run**: Completed 2026-08-25 — owner logged in via Cloudflare Access, ran a real lead through queue → analysis → complete → send, and the send succeeded. See `docs/PROJECT-STATE.md`'s Photo Check Review Portal section. This also surfaced a new blocking item below (Resend domain verification).
- [ ] **Verify a sending domain for the Review Portal's Resend account**: `review-worker`'s `RESEND_API_KEY` belongs to a Resend account/key that hasn't verified a sending domain, so it can currently only email its own account address, not real homeowner leads (confirmed via a real 422 `Invalid 'to' field` error against `@example.com` test addresses, 2026-08-25). Blocks sending real responses to real customers. Owner's action — Resend dashboard → Settings → Domains → Add Domain, same as the public form's key. See `docs/00-project-dashboard.md`'s Next section, item 0c.
